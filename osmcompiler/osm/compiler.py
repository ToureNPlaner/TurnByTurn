    ...
    def parse(self, fromblob=0, count=-1):
        ...
        while nblob < count:
            ...
            for pg in self.primblock.primitivegroup:
                #Skip processing data when the sink doesn't have the functions to do so
                if len(pg.dense.id) and self.osm_sink.what == "nodes":
                    self.processDense(pg.dense)
                if len(pg.nodes) and self.osm_sink.what == "nodes":
                    self.processNodes(pg.nodes)
                if len(pg.ways) and self.osm_sink.what == "ways":
                    self.processWays(pg.ways)
                if len(pg.relations) and hasattr(self.osm_sink, "processRelation"):
        ...
        for i in range(len(dense.id)):
            ...
            if tagloc < len(dense.keys_vals):  # don't try to read beyond the end of the list
                while dense.keys_vals[tagloc] != 0:
                    ...
                    tags[sky] = svl
            tagloc += 1
            self.osm_sink.processNode(lastID, tags, lat, lon)
    ...
    def processNodes(self,nodes):
        ...
        for nd in nodes:
            ...
            for i in range(len(nd.keys)):
                ...
                tags[sk] = sv
            self.osm_sink.processNode(nd.id, tags, lat, lon)
    ...
    def processWays(self,ways):
        ...
        for wy in ways:
            ...
            for i in range(len(wy.keys)):
                ...
                svl = self.primblock.stringtable.s[vl]
                tags[sky] = svl
            self.osm_sink.processWay(wayid, tags, nodes)
